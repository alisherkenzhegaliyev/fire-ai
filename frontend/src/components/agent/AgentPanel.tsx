...
export function AgentPanel() {
  ...
  const { clearMessages, addMessage, isLoading } = useAgentStore()
  const { submit } = useStreamingAgent()
  const [isFullscreen, setIsFullscreen] = useState(false)

  const handleSubmit = (question: string) => {
    if (!question.trim() || isLoading) return
    addMessage({ id: crypto.randomUUID(), role: 'user', text: question, timestamp: Date.now() })
    submit(question)
  }
  ...
  return (
    <div
      className={clsx(
        'fixed z-40 flex flex-col border-l border-gray-700/60 bg-gray-900 shadow-2xl animate-slide-in-right',
        isFullscreen
          ? 'inset-0 top-0'
          : 'top-14 bottom-0 right-0 w-full max-w-md'
      )}
    >
      {/* Panel Header */}
      ...
      <ChatMessages onSubmit={handleSubmit} />
